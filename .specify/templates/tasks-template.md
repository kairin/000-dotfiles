...
**Constitution Requirements**: Every generated task list MUST include tasks for:
- Local validation gates (`gh-workflow-local.sh all` and `health-check.sh --workstation-audit`)
- Documentation updates for changed behavior/status artifacts
- Verification that `./start.sh` remains the only user-facing entry point
...
- [ ] T004 Validate constitution constraints for entry points and script strategy
- [ ] T005 Identify required documentation updates (roadmap, summaries, guides)
...
- [ ] TXXX Run `./.runners-local/workflows/gh-workflow-local.sh all`
- [ ] TXXX Run `./.runners-local/workflows/health-check.sh --workstation-audit`
- [ ] TXXX Verify `ROADMAP.md` and status artifacts reference only active/real files
