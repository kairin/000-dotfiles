...
`setup ship` requires `CODACY_PROJECT_TOKEN` to be exported (typically loaded
by direnv from `.envrc.local`) and `gh` to be authenticated. If `.envrc.local`
is missing the export — for example because it was generated by an older
single-token setup run — run `./setup repair-codacy-env` to regenerate the
managed block from existing token files in `~/.codacy/`, then `direnv allow .`
to reload the environment. It runs `gh pr update-branch` automatically when
the branch is BEHIND `main`, re-uploads the Codacy SARIF for the new HEAD and
base SHA so the diff comparison is fresh, then polls the four required checks
