...
_measure_zsh_startup() {
    ...
    for i in $(seq 1 $runs); do
        ...
        local seconds
        seconds=${time_output//[^0-9.]/}
        local ms
        ms=$(echo "$seconds * 1000" | bc 2>/dev/null || echo "0")
        total=$(echo "$total + $ms" | bc 2>/dev/null || echo "$total")
    done

    # Calculate average
    local average
    average=$(echo "scale=0; $total / $runs" | bc 2>/dev/null || echo "0")
...
_add_config_section() {
    ...
    local start_marker="# >>> dotfiles-config:${section_name} >>>"
    local end_marker="# <<< dotfiles-config:${section_name} <<<"
...
# Generated by 000-dotfiles configure_zsh.sh
...
configure_update_aliases() {
    ...
    local alias_config="# Update management aliases (000-dotfiles)"
...
optimize_zsh_performance() {
    ...
    # 1. Compile .zshrc
    if command -v zsh &> /dev/null; then
        if zsh -c "zcompile ${ZSHRC_FILE}" 2>/dev/null; then
    ...
    local lazy_load
    lazy_load=$(cat <<'EOF'
# Lazy load slow tools (performance optimization)
