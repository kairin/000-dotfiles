...
class WorkflowTests(DotfilesTestCase):
    ...
    def test_workflow_runs_tests_and_uploads_coverage(self):
        ...
        # Use project token only for coverage upload (simplest and most reliable auth)
        ...
        # Do not include account-level tokens — project token is sufficient for coverage upload
        self.assertNotIn("CODACY_ACCOUNT_TOKEN", workflow)
        self.assertNotIn("CODACY_API_TOKEN", workflow)
        self.assertNotIn("CODACY_ORGANIZATION_PROVIDER", workflow)
        self.assertNotIn("CODACY_USERNAME", workflow)
        self.assertNotIn("CODACY_PROJECT_NAME", workflow)
