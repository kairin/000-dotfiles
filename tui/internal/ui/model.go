...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case methodSelectedMsg:
		...
		if m.selectedTool != nil {
			...
			// Save preference if requested (for multi-method tools)
			if msg.savePreference {
				prefStore := config.NewPreferenceStore()
				_ = prefStore.SetToolMethod(m.selectedTool.ID, msg.method)
			}
...
// For multi-method tools, shows method selector first if no preference saved
...
func (m Model) startInstaller(tool *registry.Tool, resume bool, forceReinstall bool) (tea.Model, tea.Cmd) {
	...
	// Check for multi-method support
	if tool.SupportsMultipleMethods() {
		// Check for saved preference
		prefStore := config.NewPreferenceStore()
		savedMethod, _ := prefStore.GetToolMethod(tool.ID)
		...
		recommendation := detector.RecommendInstallMethod(tool.ID, sysInfo)
...
func (m Model) handleEnter() (tea.Model, tea.Cmd) {
	...
	case ViewDashboard:
		...
		if m.mainCursor < tableToolCount {
			...
			// Selected a menu tool (Feh) - go to tool detail view
...
func (m Model) viewDashboard() string {
	...
	header := HeaderStyle.Render("System Installer • Dotfiles Manager • Dev Tools")
...
// getMenuTools returns tools that should appear as menu items (Feh)
